...
class Depan extends Component {
  constructor(props) {
    ...
    this.url = "http://192.168.10.162/php-api/api.php";
  }
  componentDidMount(){
      this.ambilListData()
  }
  ...
  async klikInput(){
    if(this.state.nama == '' && this.state.alamat == '' && this.state.idEdit==null){
      alert('Silakan masukkan nama dan alamat');
    }else{        
      alert('Klik Tombol Input Sekali lagi');
        this.setState({nama:''});
        this.setState({alamat:''});
        this.setState({idEdit:null});        
        this.ambilListData();
